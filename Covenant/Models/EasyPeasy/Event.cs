    ...
    public class DownloadEvent : Event
    {
        ...
        public bool WriteToDisk()
        {
            try
            {
                ...
                {
                    File.WriteAllBytes(
                        Path.Combine(Common.EasyPeasyDownloadDirectory, Utilities.GetSanitizedFilename(this.FileName)),
                        contents
                    );
                }
                else
                {
                    using (var stream = new FileStream(Path.Combine(Common.EasyPeasyDownloadDirectory, Utilities.GetSanitizedFilename(this.FileName)), FileMode.Append))
                    {
