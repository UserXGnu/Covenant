...
namespace EasyPeasy.Models.Grawls
{
    public class GrawlTask : ISerializable<GrawlTask>
    {
        ...
        {
            return File.ReadAllBytes(Common.EasyPeasyTaskCSharpCompiledNet35Directory + this.Name + ".compiled");
        }

        public byte[] GetCompressedILAssembly40()
        {
            return File.ReadAllBytes(Common.EasyPeasyTaskCSharpCompiledNet40Directory + this.Name + ".compiled");
        }
        ...
        private void CompileDotNet35()
        {
            List<Compiler.EmbeddedResource> resources = this.EmbeddedResources.Select(ER =>
            {
                ...
                {
                    Name = ER.Name,
                    File = Common.EasyPeasyEmbeddedResourcesDirectory + ER.Location,
            ...
            this.ReferenceSourceLibraries.ToList().ForEach(RSL =>
            {
                references35.AddRange(
                    ...
                    {
                        return new Compiler.Reference { File = Common.EasyPeasyAssemblyReferenceDirectory + RA.Location, Framework = Common.DotNetVersion.Net35, Enabled = true };
                    })
                );
            });
            references35.AddRange(
            ...
            File.WriteAllBytes(Common.EasyPeasyTaskCSharpCompiledNet35Directory + this.Name + ".compiled",
                Utilities.Compress(Compiler.Compile(new Compiler.CsharpFrameworkCompilationRequest
                {
                    Language = this.Language,
                    Source = this.Code,
                    SourceDirectories = this.ReferenceSourceLibraries.Select(RSL => Common.EasyPeasyReferenceSourceLibraries + RSL.Location).ToList(),
        ...
        private void CompileDotNet40()
        {
            ...
            this.ReferenceSourceLibraries.ToList().ForEach(RSL =>
            {
                references40.AddRange(
                    ...
                    {
                        return new Compiler.Reference { File = Common.EasyPeasyAssemblyReferenceDirectory + RA.Location, Framework = Common.DotNetVersion.Net40, Enabled = true };
                    })
                );
            });
            references40.AddRange(
            ...
            File.WriteAllBytes(Common.EasyPeasyTaskCSharpCompiledNet40Directory + this.Name + ".compiled",
                Utilities.Compress(Compiler.Compile(new Compiler.CsharpFrameworkCompilationRequest
                {
                    Language = this.Language,
                    Source = this.Code,
...
@"<Reference Include=""{0}"">
      <HintPath>{1}</HintPath>
            ...
            string dir = Common.EasyPeasyDataDirectory + "Grawl" + Path.DirectorySeparatorChar + sanitizedName + Path.DirectorySeparatorChar + "Task" + Path.DirectorySeparatorChar;
            ...
            File.WriteAllBytes(Common.EasyPeasyTaskCSharpCompiledNetCoreApp30Directory + this.Name + ".compiled",
